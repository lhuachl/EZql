...
export class EZqlClient implements IEZqlClient, IEventEmitter {
  ...
  constructor(
    ...
    // Crear dependencias - en un sistema real vendría de DI container
  ...
  static create(
    ...
    return new EZqlClient(connectionConfig, queryConfig, options);
  }
  ...
  async raw<T = any>(query: string, ...parameters: SqlValue[]): Promise<T[]> {
    ...
    try {
      this.emit('query.start', { sql: query, parameters });
      this.logger?.debug('Executing raw query', { sql: query, parameters });
      ...
      this.emit('query.complete', { sql: query, parameters, result, executionTime });
      ...
      const executionTime = Date.now() - startTime;
      this.emit('query.error', { sql: query, parameters, error, executionTime });
      this.metricsCollector?.incrementCounter('ezql.query.errors');
      this.logger?.error('Raw query failed', error as Error, { sql: query, parameters });
      throw error;
    }
  }
